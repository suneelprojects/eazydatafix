...
class Report:
    ...
    def summary(self) -> None:
        ...
        Console.key_value(
            "Completeness",
            f"{self._report.completeness.completeness_score:.2f}%",
        )
        Console.key_value(
            "Uniqueness",
            f"{self._report.uniqueness.uniqueness_score:.2f}%",
        )

        Console.section("💡 RECOMMENDATIONS")

        if not self._report.recommendations:
            Console.key_value("Status", "No recommendations. Dataset looks healthy. ✅")
        else:
            for index, recommendation in enumerate(
                self._report.recommendations,
                start=1,
            ):
                Console.recommendation(index, recommendation.title)
                Console.key_value(
                    "Priority",
